...
void DevDuino_TCS3472::displayRGBC(unsigned int r, unsigned int g, unsigned int b, unsigned int c, unsigned int integrationTime)
{
	...
	if ((deviceID == 0x44))
	  {
		display.print(String("TCS34725"), 35, 55, &defaultFont);
	  }
	else if ((deviceID == 0x4D))
      {
		display.print(String("TCS34727"), 35, 55, &defaultFont);
	  }
	...
	if (r==maxResolution || g==maxResolution || b==maxResolution || c==maxResolution)
	  {
		display.clear();  
		...
		display.setScale(1);
        display.print(String(maxResolution), 40, 10, &defaultFont);		
		display.flush();
	  }  
   else
	  {  
		...
		if (r>=b && r>=g)
		{
		  ...
		  if (b>=g)
		  {        
			analogWrite(LED_PINOUT_BLUE, map(b, 0,255, 255, 0));  
			analogWrite(LED_PINOUT_GREEN, map(g, 0,255, 255, 0));  
		  }
		  else
		  {
			analogWrite(LED_PINOUT_GREEN, map(g, 0,255, 255, 0));  
		...
		else if (g>=b && g>=r)
		{
		  ...
		  {        
			analogWrite(LED_PINOUT_BLUE, map(b, 0,255, 255, 0));  
			analogWrite(LED_PINOUT_RED, map(r, 0,255, 255, 0));  
		  }
		  else
		  {
			analogWrite(LED_PINOUT_RED, map(r, 0,255, 255, 0));  
